...
public class DownloadHttpResponseHandler extends AsyncHttpResponseHandler {
	...
	public void addAllowedContentTypes(String contentType) {
		...
		System.arraycopy(mAllowedContentTypes, 0, types, 0,
				mAllowedContentTypes.length);
	...
	public void sendResponseMessage(HttpResponse response,
		...
		try {
			...
			if (!parent.exists() && !parent.mkdirs()) {
				sendFailureMessage(new IOException("create new path failure!"),
						parent.getAbsolutePath());
				return;
			}
			if (!file.exists() && !file.createNewFile()) {
				sendFailureMessage(new IOException("create new file failure!"),
						mSavePath);
				return;
			}
