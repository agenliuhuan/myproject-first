...
package mobi.dlys.android.core.net.http.request;
...
public class AsyncHttpRequest implements Runnable {
	...
	protected boolean isBinaryRequest;
	...
	public void run() {
		try {
			...
			LogUtils.e(TAG, "thread name:" + Thread.currentThread().getName());
	...
	private void makeRequest() throws IOException {
		if (!Thread.currentThread().isInterrupted()) {
			try {
				...
				if (!Thread.currentThread().isInterrupted()) {
					if (responseHandler != null) {
						responseHandler.sendResponseMessage(response, request);
					}
				} else {
				...
				e.printStackTrace();
				LogUtils.e(TAG, "URI:" + request.getURI());
				LogUtils.e(TAG, "RequestLine:" + request.getRequestLine());
				if (!Thread.currentThread().isInterrupted()) {
					throw e;
				}
			}
