...
public abstract class AsyncTask<Params, Progress, Result> {
	...
	static {
		LogUtils.i(LOG_TAG, "CPU ： " + CPU_COUNT);
	}
	/*********************************** 基本线程池（无容量限制） *******************************/
	/**
	 * 有N处理器，便长期保持N个活跃线程。
	...
	/**
	 * An {@link Executor} that can be used to execute tasks in parallel. 核心线程数为
	 * {@link #CORE_POOL_SIZE}，不限制并发总线程数! 这就使得任务总能得到执行，且高效执行少量（<=
	 * {@link #CORE_POOL_SIZE}）异步任务。 线程完成任务后保持{@link #KEEP_ALIVE}
	 * 秒销毁，这段时间内可重用以应付短时间内较大量并发，提升性能。 它实际控制并执行线程任务。
	 */
	protected static final ThreadPoolExecutor mCachedSerialExecutor = new ThreadPoolExecutor(
	...
	private static class SmartSerialExecutor implements Executor {
		 ...
		 * 
		 * <p>
		 * cpu count : 1 2 3 4 8 16 32 64
		 * <p>
		 * base(cpu+1) : 2 3 4 5 9 17 33 65
		 * <p>
		 * once(base*2): 4 6 8 10 18 34 66 130
		 * 
		 * <p>
		 * 一个时间段内最多并发线程个数： 单核手机：4 双核手机：6 四核手机：8 ... 计算公式如下：
		 */
		 ...
		 * cpu count : 1 2 3 4 8 16 32 64
		 * <p>
		 * base(cpu+3) : 4 5 6 7 11 19 35 67
		 * <p>
		 * max(base*16): 64 80 96 112 176 304 560 1072
		 */
	 ...
	 * <p>
	 * 加载类似瀑布流时产生的大量并发（一定程度允许任务被剔除队列）时请用
	...
	/**
	 * <p>
	 * 用于瞬间大量并发的场景，比如，假设用户拖动{@link ListView}
	 * 时如果需要加载大量图片，而拖动过去时间很久的用户已经看不到，允许任务丢失。
	 * <p>
	 ...
	 * <p>
	 * 用于重要、紧急、单独的异步任务，该Runnable立即得到执行。
	 * <p>
	 * 加载类似瀑布流时产生的大量并发（任务数超出限制允许任务被剔除队列）时请用
	 ...
	 * <p>
	 * 用于瞬间大量并发的场景，比如，假设用户拖动{@link ListView}
	 * 时如果需要启动大量异步线程，而拖动过去时间很久的用户已经看不到，允许任务丢失。
	 * <p>
	 * This method execute runnable wisely when a large number of task will be
	 * submitted.
	 * <p>
	 * 任务数限制情况见{@link SmartSerialExecutor} immediate execution for important or
	 * urgent task.
	 * 
	 * @param runnable
	 */
