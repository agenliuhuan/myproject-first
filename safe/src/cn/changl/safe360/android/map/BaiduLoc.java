...
public class BaiduLoc implements OnGetGeoCoderResultListener {
	public static final int MAP_SCAN_SPAN_FOR_TRIP = 30 * 1000;
	public static final int MAP_SCAN_SPAN_FOR_FOREGROUND = 60 * 1000;
	public static final int MAP_SCAN_SPAN_FOR_BACKGROUND = 3 * 60 * 1000;

	public enum MapScanType {
		ET_TRIP, ET_FOREGROUND, ET_BACKGROUND
	}
	...
	private int mScanSpan = MAP_SCAN_SPAN_FOR_FOREGROUND;
	...
	public void changeMapScanType(MapScanType scanType) {
		switch (scanType) {
		case ET_TRIP:
			if (MAP_SCAN_SPAN_FOR_TRIP == mScanSpan) {
				return;
			}
			mScanSpan = MAP_SCAN_SPAN_FOR_TRIP;
			break;
		case ET_FOREGROUND:
			if (MAP_SCAN_SPAN_FOR_FOREGROUND == mScanSpan) {
				return;
			}
			mScanSpan = MAP_SCAN_SPAN_FOR_FOREGROUND;
			break;
		case ET_BACKGROUND:
			if (MAP_SCAN_SPAN_FOR_BACKGROUND == mScanSpan) {
				return;
			}
			mScanSpan = MAP_SCAN_SPAN_FOR_BACKGROUND;
			break;
		default:
			break;
		}
	...
	public void startNotify(BDNotifyListener listener) {
		notifyListener = listener;
		mLocationClient.registerNotify(notifyListener);
	}

	public void stopNotify() {
		if (notifyListener != null && mLocationClient != null) {
