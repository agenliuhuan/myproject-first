package cn.changl.safe360.android.ui.comm.slidingmenu.lib;
...
import cn.changl.safe360.android.ui.comm.HorizontalListView;
import cn.changl.safe360.android.ui.comm.slidingmenu.lib.SlidingMenu.OnClosedListener;
import cn.changl.safe360.android.ui.comm.slidingmenu.lib.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	private boolean checkChildView(Object v, MotionEvent ev) {
		ViewGroup vg = null;
		...
		for (int i = 0; i < vg.getChildCount(); i++) {
			if (vg.getChildAt(i) instanceof HorizontalListView || vg.getChildAt(i) instanceof HorizontalScrollView) {
				if (inRangeOfView(vg.getChildAt(i), ev)) {
					((ViewGroup) vg.getChildAt(i)).onInterceptTouchEvent(ev);
					return true;
				}
			}

			if (checkChildView(vg.getChildAt(i), ev)) {
				return true;
			}
		}
