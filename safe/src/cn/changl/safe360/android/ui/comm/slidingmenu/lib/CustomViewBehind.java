...
import cn.changl.safe360.android.ui.comm.slidingmenu.lib.SlidingMenu.CanvasTransformer;

public class CustomViewBehind extends ViewGroup {

	private static final String TAG = "CustomViewBehind";

	private static final int MARGIN_THRESHOLD = 28; // dips
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mSecondaryContent != null)
			mSecondaryContent.measure(contentWidth + mWidthOffset, contentHeight);
	}
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			if (x <= content.getLeft()) {
				...
				scrollTo((int) (getBehindWidth() - getWidth() + mWidthOffset), y);
				// scrollTo((int) (getBehindWidth() - getWidth() + (x -
				// getBehindWidth()) * mScrollScale), y);
			}
		}
	...
	public int getMenuLeft(View content, int page) {
		if (mMode == SlidingMenu.LEFT) {
			...
			case 0:
				return content.getLeft() - getBehindWidth();
			case 2:
				return content.getLeft() + getBehindWidth() + mWidthOffset;
			}
		}
		return content.getLeft();
	}
