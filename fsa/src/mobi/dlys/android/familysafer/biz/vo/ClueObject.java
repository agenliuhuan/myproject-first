 ...
 * 
 */
public class ClueObject extends BaseObject {
	private static final long serialVersionUID = 5790773689850017190L;
	...
	// 图片列表
	@RowType(clz = ClueImageObject.class)
	...
	public static ClueObject createFromPb(Clue clue) {
		...
		if (clue != null) {
			...
			if (clue.getImagesList() != null && clue.getImagesCount() > 0) {
				...
				for (ClueImage clueImage : clue.getImagesList()) {
					...
					imageObject.setInsertTime(System.nanoTime());
			...
			clueObject.setEvent(clue.getIsEvent());
	...
	public void setClueId() {
		if (imageList != null) {
			for (ClueImageObject clueImage : imageList) {
				clueImage.setClueId(id);
				clueImage.setInsertTime(System.nanoTime());
			}
		}
	}
