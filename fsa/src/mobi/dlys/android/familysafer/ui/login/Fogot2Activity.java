...
public class Fogot2Activity extends BaseExActivity {
	...
	private void initView() {
		...
		mPhoneTip = (TextView) findViewById(R.id.tv_fogot_2_tip_1);
		mPhoneTip.setText(getString(R.string.activity_fogot_2_tv_tip_1) + bl.getString("phone") + "，");
		...
		mResend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				tips += getString(R.string.activity_fogot_2_tv_tip_3);
		...
		case YSMSG.RESP_VERIFY_AUTH_CODE: {
			...
			if (msg.arg1 == 200) {
				...
				if (CoreModel.getInstance().getUserInfo() != null) {
					...
					userInfo.setPhone(CoreModel.getInstance().getUserInfo().getPhone());
					userInfo.setAuthCode(mCheckNumber.getEditableText().toString());
